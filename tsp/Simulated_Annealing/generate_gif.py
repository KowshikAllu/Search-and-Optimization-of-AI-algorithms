...
def create_enhanced_frame(tour, points, frame_num, total_frames):
    ...
    for i, (x, y) in enumerate(zip(x_coords, y_coords)):
        ax.annotate(str(i), (x, y), textcoords="offset points", xytext=(0, 5), ha='center', fontsize=8)
    ...
    ax.set_title(f"TSP Simulated Annealing Solution\nStep {min(frame_num, len(tour))}/{len(tour)}")
...
def generate_gif(tour, points, output="simulated_annealing.gif"):
    if not tour or len(tour) != len(points):
        raise ValueError("Invalid tour or points data")

    print("\nGenerating Simulated Annealing GIF...")
    frame_paths = []
    total_frames = len(tour) + 2
...
if __name__ == "__main__":
    ...
    try:
        ...
        best_tour = [int(i) for i in np.loadtxt("best_sa_tour.txt")]
        generate_gif(best_tour, problem.points)
    except Exception as e:
        print(f"Error: {e}")
